...
Metadata.prototype.buildTokens = function (hosts) {
  ...
  hostArray.forEach(function (h) {
    ...
    h.tokens.forEach(function (tokenString) {
      const token = self.tokenizer.parse(tokenString);
      utils.insertSorted(allSorted, token, compare);
      primaryReplicas[stringify(token.getValue())] = h;
    });
  ...
  for (let i = 0; i < allSorted.length; i++) {
    this.ringTokensAsStrings[i] = stringify(allSorted[i].getValue());
  }
 ...
 * @returns {Set<TokenRange>} Ranges for the keyspace on this host
 */
Metadata.prototype.getTokenRangesForHost = function (keyspaceName, host) {
  ...
  // TODO: This code is duplicated from getReplicas, consider making common method.
  if (!keyspace.replicas) {
    //Calculate replicas the first time for the keyspace
    keyspace.replicas =
      keyspace.tokenToReplica(this.tokenizer, this.ringTokensAsStrings, this.primaryReplicas, this.datacenters);
  ...
  // TODO: Consider caching/precomputing keyspace -> (host, range) as this could be an expensive operation.
  const ranges = new Set();
  // find tokens containing host and identify ranges beginning with that token.
  Object.keys(keyspace.replicas).forEach((tokenStr) => {
    const replicas = keyspace.replicas[tokenStr];
    if(replicas.includes(host)) {
      // identify ranges that start with this token.
      const token = self.tokenizer.parse(tokenStr);
      this.tokenRanges.forEach((tokenRange) => {
        if (tokenRange.start.equals(token)) {
